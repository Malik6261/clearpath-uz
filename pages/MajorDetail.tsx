...
const MajorDetail: React.FC = () => {
  ...
  if (!major) return <Navigate to={`/majors`} />;
  ...
  return (
    <div className="bg-stone-50 min-h-screen">
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-32">
        {/* Back Link */}
        <nav className="mb-20">
          <Link to={`/majors`} className="group inline-flex items-center gap-2 text-[11px] font-bold text-stone-400 hover:text-stone-900 uppercase tracking-widest transition-colors">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-stone-200 rounded-[2.5rem] overflow-hidden border border-stone-200">
              {major.realityCheck.map((row, i) => (
                <div key={i} className="bg-white p-10 hover:bg-stone-50 transition-colors">
                  ...
                  <p className="text-lg font-bold text-stone-900 leading-snug mb-2">Tahlil</p>
        ...
        <section className="py-40 text-center border-t border-stone-100">
           ...
           <Link to={`/majors`} className="px-12 py-5 bg-stone-900 text-stone-50 rounded-full font-bold text-[13px] uppercase tracking-widest hover:scale-105 transition-all">
              {t.returnToDirectory}
           </Link>
        </section>
      </div>
